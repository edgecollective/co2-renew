...
# Current CO2 value label displayed at top middle of screen
current_co2_label = label.Label(
    font=font, text="---", color=line_color, scale=2
)
current_co2_label.anchor_point = (0.5, 0)  # center-top anchor
current_co2_label.anchored_position = (WIDTH // 2 + 12, 2)  # Top middle position shifted right
                ...
                if(SCREEN==0):
                    ...
                    if all_values:
                        ...
                        if needs_rescale:
                            ...
                            new_y_min = max(400, min_co2 - range_padding)
                    ...
                    text_label1b.text=str(400)  # Always display 400 as minimum
                    
                    # Calculate midpoint between max and 400, round up to nearest 100
                    midpoint = (sparkline1.y_top + 400) / 2
                    rounded_midpoint = int(math.ceil(midpoint / 100) * 100)
                    text_label1c.text=str(rounded_midpoint)
                    
                    # Position the middle label at the correct y-coordinate for the rounded midpoint value
                    data_range = sparkline1.y_top - sparkline1.y_bottom
                    if data_range > 0:
                        # Calculate where the rounded_midpoint should appear on the chart
                        normalized_position = (rounded_midpoint - sparkline1.y_bottom) / data_range
                        middle_y = sparkline1.y + chart_height - (normalized_position * chart_height)
                        text_label1c.anchored_position = (
                            sparkline1.x + text_xoffset,
                            int(middle_y)
                        )
                    
                    # Update current CO2 value label (position is fixed at top middle)
