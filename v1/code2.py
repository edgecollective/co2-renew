...
my_group.append(top_tickmark)
my_group.append(middle_tickmark)
my_group.append(bottom_tickmark)
...
# Create tickmarks - top and bottom are fixed, middle will be updated dynamically
top_tickmark = Line(sparkline1.x - 5, sparkline1.y, sparkline1.x, sparkline1.y, color=line_color)
bottom_tickmark = Line(sparkline1.x - 5, sparkline1.y + chart_height, sparkline1.x, sparkline1.y + chart_height, color=line_color)
# Middle tickmark will be positioned dynamically
middle_tickmark = Line(sparkline1.x - 5, sparkline1.y + chart_height//2, sparkline1.x, sparkline1.y + chart_height//2, color=line_color)
                ...
                if(SCREEN==0):
                    ...
                    if all_values:
                        ...
                        if needs_rescale:
                            ...
                            new_y_min = max(400, min_co2 - range_padding)  # Always keep minimum at 400
                            new_y_max = max_co2 + range_padding
                            
                            # Round to nice numbers - y_max to multiples of 200, y_min fixed at 400
                            new_y_min = 400  # Always set minimum to 400
                            new_y_max = math.ceil(new_y_max / 200) * 200
                            
                            # Ensure minimum range of 400 for readability (since min is 400, max should be at least 800)
                            if new_y_max - new_y_min < 400:
                                new_y_max = 800  # Set to next multiple of 200
                    ...
                    # Display ticks as multiples of 200, starting from 400
                    bottom_tick = 400  # Show actual y_bottom (400) as bottom tick
                    top_tick = sparkline1.y_top  # Top tick is already a multiple of 200
                    # Calculate middle tick as halfway between min and max, rounded to nearest 200
                    middle_value = (400 + sparkline1.y_top) // 2
                    middle_tick = round(middle_value / 200) * 200
                    
                    text_label1a.text=str(top_tick)
                    text_label1b.text=str(bottom_tick)
                    text_label1c.text=str(middle_tick)
                    
                    # Update middle label position to match its value on the scale
                    y_range = sparkline1.y_top - sparkline1.y_bottom
                    if y_range > 0:
                        # Calculate where the middle_tick value should be positioned
                        normalized_middle = (middle_tick - sparkline1.y_bottom) / y_range
                        middle_y_pos = sparkline1.y + chart_height - (normalized_middle * chart_height)
                        text_label1c.anchored_position = (
                            sparkline1.x + text_xoffset,
                            int(middle_y_pos)
                        )
                        
                        # Update middle tickmark position to match the label
                        middle_tickmark.y1 = int(middle_y_pos)
                        middle_tickmark.y2 = int(middle_y_pos)
